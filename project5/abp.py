...
def run(args, exp_dir, logger):
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.start == 'warm':
            ...
            # cold
            # TODO
            z = model.sample_langevin(z_0, data)
            pass
        ...
        loss = F.mse_loss(x_hat, data, reduction='sum')
        ...
        if (epoch + 1) % args.n_plot == 0:
            ...
            if args.start == 'warm':
                ...
                z_sampled = Variable(sample_gaussian_prior(
                    batch_size=batch_size, nz=args.nz, sig=args.prior_sigma
                ))
                z_sampled = model.sample_langevin(z_sampled, data)
                pass
            else:
                # cold
                # TODO, variable named `z_sampled`
